...
def test_install_writes_the_eval_time_files(tmp_path):
    ...
    assert (tmp_path / "nix" / "pins" / "fetchers" / "tarball.nix").exists()
    assert (tmp_path / "nix" / "pins" / "fetchers" / "git.nix").exists()
...
def test_fetchers_are_primitives_not_source_types(tmp_path):
    ...
    have = {p.stem for p in (tmp_path / "nix" / "pins" / "fetchers").glob("*.nix")}
...
def test_the_vendored_tree_is_importable_on_its_own(tmp_path):
    """A fresh clone builds with Nix alone: nix/pins must be self-contained."""
    vendor.install(tmp_path)
    assert (tmp_path / "nix" / "pins" / "default.nix").exists()
...
def test_reinstall_is_idempotent(tmp_path):
    ...
    target = tmp_path / "nix" / "pins" / "resolve.nix"
